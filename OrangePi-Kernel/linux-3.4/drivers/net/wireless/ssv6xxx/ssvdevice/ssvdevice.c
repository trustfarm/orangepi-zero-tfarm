...
char* ssv_initmac = 0;  // temp mac address if users want to use instead of the mac address in Efuse
module_param(ssv_initmac, charp, 0644);
char* ssv_custom_default_mac = 0;  // use ssv_custom_default_mac if the mac address in Efuse is not set
module_param(ssv_custom_default_mac, charp, 0644);
#endif
...
#endif
{
	...
	if (!debugfs)
		return -ENOMEM;
	debugfs_create_u32(DEBUG_DEVICETYPE_ENTRY, S_IRUGO | S_IWUGO, debugfs,
			   &ssv_devicetype);
	debugfs_create_file(DEBUG_CMD_ENTRY, S_IRUGO | S_IWUGO, debugfs, NULL,
			    &ssv6xxx_dbg_fops);
#endif
