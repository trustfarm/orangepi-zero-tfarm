...
static int ssv_cmd_mac(int argc, char *argv[])
{
	...
	if (argc == 3 && !strcmp(argv[1], "wsid") && !strcmp(argv[2], "show")) {
		...
		strcat(ssv6xxx_result_buf, temp_str);
		sprintf(temp_str, "    Total Rx frame: %d\n",
			ssv_dbg_sc->rx.num_pkts);
		strcat(ssv6xxx_result_buf, temp_str);
		return 0;
	}
#if 0
...
static int ssv_cmd_version(int argc, char *argv[])
{
	...
	*((u32 *)&chip_id[12]) = (u32)LONGSWAP(regval);
	...
	strcat(ssv6xxx_result_buf, temp_str);
	sprintf(temp_str, "# current firmware version: %d\n", ssv_root_version);
	strcat(ssv6xxx_result_buf, temp_str);
...
struct _ssv6xxx_txtput *ssv6xxx_txtput, ssv_txtput;
static int txtput_thread_m2(void *data)
{
	...
	while (!kthread_should_stop() && ssv6xxx_txtput->loop_times > 0) {
		...
		ssv_dbg_ctrl_hci->shi->hci_ops->hci_tx(skb, 0, 0);
	}
	return 0;
}
...
static int ssv_cmd_loadfw(int argc, char *argv[])
{
	return ssv6xxx_debug_ifops->ifops->load_fw(ssv6xxx_debug_ifops->dev);
}
struct ssv_cmd_table cmd_table[] = {
	...
	{ "loadfw", ssv_cmd_loadfw, "ssv6200 load fw" },
