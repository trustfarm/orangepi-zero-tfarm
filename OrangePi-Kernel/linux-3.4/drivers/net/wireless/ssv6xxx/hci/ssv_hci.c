...
{
	ctrl_hci->shi->if_ops->irq_setmask(ctrl_hci->shi->dev,
					   ~(ctrl_hci->int_mask));
	ctrl_hci->shi->if_ops->irq_enable(ctrl_hci->shi->dev);
	return 0;
}
static int ssv6xxx_hci_irq_disable(void)
{
	ctrl_hci->shi->if_ops->irq_setmask(ctrl_hci->shi->dev, 0xffffffff);
...
bool ssv6xxx_hci_init_debugfs(struct dentry *dev_deugfs_dir)
{
	...
	debugfs_create_u32("hci_isr_mib_reset", 00666, ctrl_hci->debugfs_dir,
...
irqreturn_t ssv6xxx_hci_isr(int irq, void *args)
{
	...
	do {
		...
		struct timespec tx_io_start_time, tx_io_end_time,
		    tx_io_diff_time;
		...
		if (status & SSV6XXX_INT_RX) {
			...
			if (ctrl_hci->isr_summary_eable &&
				...
				if (ctrl_hci->isr_rx_idle_time) {
					ctrl_hci->isr_rx_idle_time +=
					    (jiffies -
					     ctrl_hci->prev_rx_isr_jiffes);
					ctrl_hci->isr_rx_idle_time =
					    ctrl_hci->isr_rx_idle_time >> 1;
				} else {
					ctrl_hci->isr_rx_idle_time +=
					    (jiffies -
					     ctrl_hci->prev_rx_isr_jiffes);
			...
			if (ctrl_hci->isr_summary_eable) {
				if (ctrl_hci->isr_rx_time) {
					ctrl_hci->isr_rx_time +=
					    (jiffies - before);
					ctrl_hci->isr_rx_time =
					    ctrl_hci->isr_rx_time >> 1;
				} else {
					ctrl_hci->isr_rx_time +=
					    (jiffies - before);
				}
		...
		if (status & (SSV6XXX_INT_RESOURCE_LOW)) {
			...
			for (q_num = SSV_HW_TXQ_NUM - 1; q_num >= 0; q_num--) {
				...
				if (ctrl_hci->isr_mib_enable) {
					...
					tx_io_diff_time = timespec_sub(
					    tx_io_end_time, tx_io_start_time);
					ctrl_hci->isr_tx_io_time +=
				...
				if (ctrl_hci->isr_summary_eable) {
					if (ctrl_hci->isr_tx_time) {
						ctrl_hci->isr_tx_time +=
						    (jiffies - before);
						ctrl_hci->isr_tx_time =
						    ctrl_hci->isr_tx_time >> 1;
					} else {
						ctrl_hci->isr_tx_time +=
						    (jiffies - before);
					}
