...
void efuse_read_all_map(struct ssv_hw *sh)
{
	...
	extern char* ssv_custom_default_mac;
	u8 mac[ETH_ALEN] = {0};
	int jj,kk;
#endif
	extern struct ssv6xxx_cfg ssv_cfg;
	...
	ssv_cfg.r_calbration_result =
	    (u8)SSV_EFUSE_ITEM_TABLE[EFUSE_R_CALIBRAION_RESULT].value;
	ssv_cfg.sar_result = (u8)SSV_EFUSE_ITEM_TABLE[EFUSE_SAR_RESULT].value;
	ssv_cfg.crystal_frequency_offset =
	    (u8)SSV_EFUSE_ITEM_TABLE[EFUSE_CRYSTAL_FREQUECY_OFFSET].value;
	...
	if(ssv_initmac)
	{
		printk(KERN_ERR "mac from cmdline=%s\n",ssv_initmac);
	...
	//2: from efuse
	{
		printk(KERN_ERR "mac from efuse=%02x:%02x:%02x:%02x:%02x:%02x\n",
				rom_mac0[0],rom_mac0[1],rom_mac0[2],
				rom_mac0[3],rom_mac0[4],rom_mac0[5]);
...
#ifdef ALLWINNER_H3_SUPPORT
	//3: from custom default
	if (ssv_custom_default_mac)
	{
		printk(KERN_ERR "mac from custom default=%s\n",ssv_custom_default_mac);
		for( jj = 0, kk = 0; jj < ETH_ALEN; jj++, kk += 3 )
		{
			mac[jj] = key_2char2num(ssv_custom_default_mac[kk], ssv_custom_default_mac[kk+1]);
		}
		if(is_valid_ether_addr(mac)) {
			memcpy(&sh->cfg.maddr[0][0],mac,ETH_ALEN);
			addr_increase_copy(&sh->cfg.maddr[1][0],mac);
	...
	//4: from chip_id
	{
		...
		printk(KERN_ERR "mac from chip_id=%02x:%02x:%02x:%02x:%02x:%02x\n",
				mac[0],mac[1],mac[2],
				mac[3],mac[4],mac[5]);
		if(is_valid_ether_addr(mac))
		{
