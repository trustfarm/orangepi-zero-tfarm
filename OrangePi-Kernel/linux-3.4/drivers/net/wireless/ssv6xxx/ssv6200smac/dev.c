...
int ssv6xxx_set_channel(struct ssv_softc *sc, int ch)
{
    ...
    if(sc->sh->cfg.chip_identity == SSV6051Z)
    {
    	if ((ch == 13) || (ch == 14)) {
    		if (sc->ipd_channel_touch == 0) {
    			SMAC_REG_WRITE(sc->sh, ADR_ABB_REGISTER_1, 0x151559fc);
                SMAC_REG_WRITE(sc->sh, ADR_LDO_REGISTER, 0x00eb7c1c);
                SMAC_REG_WRITE(sc->sh, ADR_RX_ADC_REGISTER, 0x20d000d2);
    			sc->ipd_channel_touch = 1;
    		}
    	} else {
    		if (sc->ipd_channel_touch) {
    			SMAC_REG_WRITE(sc->sh, ADR_ABB_REGISTER_1,
    				       sc->def_setting_CE01000C);
    			SMAC_REG_WRITE(sc->sh, ADR_LDO_REGISTER,
    				       sc->def_setting_CE010008);
    			SMAC_REG_WRITE(sc->sh, ADR_RX_ADC_REGISTER,
    				       sc->def_setting_CE010030);
    			sc->ipd_channel_touch = 0;
    		}
    	}
    }
    
    if(sc->sh->cfg.chip_identity == SSV6051P)
    {
        //Channel 13/14
        if((ch == 13) || (ch == 14))
        {
		 ...
		 struct ieee80211_key_conf *key)
{
	...
	*(u16 *)&sram_key->sta_key[0].reserve[0] = key->keylen;
...
#ifdef USE_LOCAL_WEP_CRYPTO
	{
		...
		vif_priv->crypt = get_crypto_wep_ops();
		vif_priv->crypt_priv = NULL;
		if (vif_priv->crypt)
			vif_priv->crypt_priv =
			    vif_priv->crypt->init(key->keyidx);
		if (vif_priv->crypt_priv) {
			vif_priv->crypt->set_key(key->key, key->keylen, NULL,
						 vif_priv->crypt_priv);
			dev_err(sc->dev,
	...
	if (sta_priv->need_sw_encrypt || sta_priv->need_sw_decrypt) {
		...
		if (temp_crypt_priv) {
			...
			temp_crypt->set_key(key->key, key->keylen, NULL,
					    temp_crypt_priv);
			sta_priv->crypt_priv = temp_crypt_priv;
			sta_priv->crypt = temp_crypt;
		} else {
			dev_err(sc->dev,
				    ...
				    struct ieee80211_key_conf *key)
{
	...
	if (vif_priv->need_sw_encrypt || vif_priv->need_sw_decrypt) {
		...
		if (temp_crypt_priv) {
			...
			temp_crypt->set_key(key->key, key->keylen, NULL,
					    temp_crypt_priv);
			vif_priv->crypt_priv = temp_crypt_priv;
			vif_priv->crypt = temp_crypt;
		} else {
			vif_priv->need_sw_encrypt = false;
			vif_priv->need_sw_decrypt = false;
			dev_err(sc->dev,
				  ...
				  struct ieee80211_key_conf *key)
{
	...
	if (sta_priv->crypt_priv) {
		...
		sta_priv->crypt->set_key(key->key, key->keylen, NULL,
					 sta_priv->crypt_priv);
	} else {
		dev_err(sc->dev,
			   ...
			   struct ieee80211_key_conf *key)
{
	...
	dev_err(sc->dev, "Set key VIF %d STA %d algorithm = %d, key->keyidx = "
			 "%d, cmd = %d\n",
	...
	if (vif_priv) {
		printk(
		    "vif: hw_en:%d, sw_en:%d, hw_de:%d, sw_de:%d, valid:%d\n",
		    (vif_priv->has_hw_encrypt == true),
		    (vif_priv->need_sw_encrypt == true),
		    (vif_priv->has_hw_decrypt == true),
		    (vif_priv->need_sw_decrypt == true),
			   ...
			   struct ieee80211_sta *sta)
{
	...
	do {
		spin_lock_irqsave(&sc->ps_state_lock, flags);
#if 1
		if ((vif_priv->vif_idx == 0) || sc->sh->cfg.use_wpa2_only)
			s = 0;
		else
			s = 2;
#else
...
#ifdef USE_LOCAL_CRYPTO
#ifdef MULTI_THREAD_ENCRYPT
void ssv6xxx_skb_get_tx_cryptops(struct sk_buff *mpdu,
				 struct ssv_crypto_ops **crypt_ptr,
				 void **crypt_priv_ptr)
{
...
int ssv6xxx_skb_encrypt(struct sk_buff *mpdu, struct ssv_softc *sc)
{
	...
	if ((crypt == NULL) || (crypt_priv == NULL)) {
		dev_err(sc->dev, "[Local Crypto]: mpdu[0x%x] Encrypt %c %d "
				 "%02X:%02X:%02X:%02X:%02X:%02X with NULL "
				 "crypto.\n",
			(unsigned int)mpdu, unicast ? 'U' : 'B', mpdu->protocol,
			hdr->addr1[0], hdr->addr1[1], hdr->addr1[2],
			hdr->addr1[3], hdr->addr1[4], hdr->addr1[5]);
		return -1;
	} else {
...
void ssv6xxx_skb_get_rx_cryptops(struct sk_buff *mpdu,
				 struct ieee80211_sta *sta,
				 struct ssv_softc *sc,
				 struct ssv_crypto_ops **crypt_ptr,
				 void **crypt_priv_ptr)
...
static void _proc_data_rx_skb(struct ssv_softc *sc, struct sk_buff *rx_skb)
{
	...
	if (rxdesc->rate_idx < SSV62XX_G_RATE_INDEX && rxphypad->RSVD == 0) {
		if ((ieee80211_is_beacon(hdr->frame_control)) ||
			...
			if (sta) {
				...
				printk("beacon %02X:%02X:%02X:%02X:%02X:%02X "
				       "rxphypad-rpci=%d\n",
				       ...
				       rxphypad->rpci);
#endif
				if (sc->wsid_rssi[rxdesc->wsid]) {
					sc->wsid_rssi[rxdesc->wsid] =
					    (rxphypad->rpci +
					     ((sc->wsid_rssi[rxdesc->wsid]
					       << 3) -
					      sc->wsid_rssi[rxdesc->wsid])) >>
		...
		sta = ssv6xxx_find_sta_by_rx_skb(sc, rx_skb);
		if (sta)
			rxs->signal = -(sc->wsid_rssi[rxdesc->wsid]);
#ifdef SSV_RSSI_DEBUG
	...
	if ((rx_hw_dec == false) && (do_sw_dec == true)) {
		...
		if (ret == 0) {
			...
			spin_unlock_irqrestore(&sc->crypt_st_lock, flags);
			list_for_each_entry_reverse(ta, &encrypt_task_head,
						    list)
			{
		   ...
		   spinlock_t *rx_q_lock)
{
	...
	while (1) {
		...
		if (rxdesc->c_type == HOST_EVENT) {
            ...
            {
                //printk("!! SOC_EVT_WATCHDOG_TRIGGER !!\n");
                dev_kfree_skb_any(skb);
                sc->watchdog_flag = 1;
            }
