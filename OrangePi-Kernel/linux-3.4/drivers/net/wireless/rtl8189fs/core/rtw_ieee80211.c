...
err_chk:
	if (rtw_check_invalid_mac_address(mac, _TRUE) == _TRUE) {
		...
		DBG_871X_LEVEL(_drv_err_, "invalid mac addr:"MAC_FMT", assign MAC from user or random.\n", MAC_ARG(mac));
		...
		{	//	Users specify the mac address
			...
			DBG_871X_LEVEL(_drv_err_, "MAC Address from custom default set "MAC_FMT"\n", MAC_ARG(mac));
		}
		else
		{
			...
			if(!is_valid_ether_addr(mac))
			{
				...
				DBG_871X_LEVEL(_drv_err_, "MAC Address from random mac is "MAC_FMT"\n", MAC_ARG(mac));
			}
			else
			{
				DBG_871X_LEVEL(_drv_err_, "MAC Address from chip id is "MAC_FMT"\n", MAC_ARG(mac));
			}
		}
		#else
