...
static int get_flash_type(void)
{
	...
	memset(ctype, 0, 16);
	if(get_para_from_cmdline(saved_command_line ,"boot_type", ctype) <= 0){
...
static int _sd_read(char *buf, ssize_t len, ssize_t offset)
{
	int ret ;
...
static int _sd_write(char *buf, ssize_t len, ssize_t offset)
{
	int ret;
		...
		unsigned long ioctl_param)
{
	...
	switch(ioctl_num){
		case SST_STORAGE_READ:
		    ...
		    wait_for_completion(&nfcr->tsk_end);
			//sunxi_dump(nfcr->temp_data, 50);
			err = nfcr->ret;
			if (!err) {
				err = copy_to_user((void __user *)nfcr->key_store.buf, nfcr->temp_data, nfcr->key_store.len);
				if (err)
...
static int sst_storage_work(void* context)
{
	fcry_pt fcpt = NULL;
...
static int __init sunxi_secure_storage_init(void)
{
	...
	nfcr->tsk = kthread_run(sst_storage_work, nfcr, "schw");
	if (IS_ERR(nfcr->tsk)) {
		pr_err("Tsk request fail!\n");
		return (-EINVAL);
	}
