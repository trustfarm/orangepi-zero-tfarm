...
static int Fb_map_video_memory(struct fb_info *info)
{
	...
	if(info->screen_base)	{
		__inf("%s(reserve),va=0x%x, pa=0x%x size:0x%x\n", __func__,
...
static inline void Fb_unmap_video_memory(struct fb_info *info)
{
	...
	__inf("%s: screen_base=0x%x, smem=0x%x, len=0x%x\n", __func__,
		(unsigned int)info->screen_base, (unsigned int)info->fix.smem_start, info->fix.smem_len);
	...
	case FBIO_FREE:
	{
		...
		if((!g_fbi.fb_enable[fb_id])
			...
			__wrn("%s, fb%d already release ?"
				" or fb_info mismatch: fbinfo=0x%x, info=0x%x\n",
				__func__, fb_id, (unsigned int)fbinfo, (unsigned int)info);
			return -1;
		}
