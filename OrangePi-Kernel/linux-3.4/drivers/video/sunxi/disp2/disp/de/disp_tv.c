...
s32 disp_tv_enable( struct disp_device* ptv)
{
	...
	ret = disp_sys_register_irq(ptvp->irq_no,0,disp_tv_event_proc,(void*)ptv->disp,0,0);
	if (ret!=0) {
...
{
	if (para == NULL) {
		printk("disp_init_tv_para is NULL");
		return DIS_FAIL;
	}
	memcpy(&g_init_para, para, sizeof(disp_bsp_init_para));
	printk("DB DISP INIT_TV_PARA\n");
	return 0;
}
...
s32 disp_init_tv(void)//disp_bsp_init_para * para)  //call by disp_display
{
	...
	if(g_tv_used) {						//modify
		...
		for(disp=0; disp<num_screens; disp++) {
			...
			printk("screen %d support TV type\n", disp);
