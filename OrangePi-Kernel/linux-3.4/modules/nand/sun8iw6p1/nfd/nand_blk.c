...
static int nand_ioctl(struct block_device *bdev, fmode_t mode,
		      unsigned int cmd, unsigned long arg)
{
	...
	case BLKBURNBOOT0:
		if (0 == down_trylock(&(nandr->nand_ops_mutex))) {
			IS_IDLE = 0;
			ret =
			    NAND_BurnBoot0(burn_param->length,
					   burn_param->buffer);
...
int add_nand_blktrans_dev_for_dragonboard(struct nand_blk_dev *dev)
{
	...
	snprintf(gd->disk_name, sizeof(gd->disk_name), "%s%c", tr->name, (1 ? 'a' : '0') + dev->devnum);
	set_capacity(gd, 512);
