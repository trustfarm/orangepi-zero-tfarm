...
int __init nand_init(void)
{
	...
	if (SCIRPT_ITEM_VALUE_TYPE_INT != type)
		NAND_Print_DBG("nand type err! %d", type);
...
#ifdef SUN8IW1P1
	NAND_Print_DBG("[NAND]run on A31\n");
#endif

#ifdef SUN8IW3P1
	NAND_Print_DBG("[NAND]run on A23\n");
#endif

#ifdef SUN9IW1P1
	NAND_Print_DBG("[NAND]run on A80\n");
#endif

#ifdef SUN8IW6P1
	NAND_Print_DBG("[NAND]run on A83\n");
#endif
	...
	if (request_irq(SUNXI_IRQ_NAND0, nand_interrupt_ch0, IRQF_DISABLED, mytr.name, &mytr)) {
		...
		NAND_Print_DBG("nand interrupte ch0 irqno: %d register ok\n", SUNXI_IRQ_NAND0);
	...
	if (request_irq(SUNXI_IRQ_NAND1, nand_interrupt_ch1, IRQF_DISABLED, mytr.name, &mytr)) {
		...
		NAND_Print_DBG("nand interrupte ch1, irqno: %d register ok\n", SUNXI_IRQ_NAND1);
...
#ifdef SUN8IW5P1
	NAND_Print_DBG("regisger ISP =====================\n");
	if (request_irq(SUNXI_IRQ_NAND, nand_interrupt_ch0, IRQF_DISABLED, mytr.name, &mytr)) {
		NAND_Print_DBG("nand interrupte ch0 irqno: %d register error\n", SUNXI_IRQ_NAND);
		return -EAGAIN;
	}
#endif
