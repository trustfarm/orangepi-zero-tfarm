...
extern void *NAND_GetPageCache(void);
...
int _is_lsb_page(__u32 page_num)
{
	...
	if ((read_retry_mode == 0x0) || (read_retry_mode == 0x1)
	    || (read_retry_mode == 0x2) || (read_retry_mode == 0x3)) {
		if ((page_num == 0) || (page_num == 1))
			return 1;
		if ((page_num % 4 == 2) || (page_num % 4 == 3)) {
			if ((page_num != (pages_per_block - 1))
			    && (page_num != (pages_per_block - 2)))
				return 1;
		}
		return 0;
...
__s32 burn_boot0_1k_mode_secure(__u32 read_retry_type, __u32 length, __u32 Boot0_buf)
{
    ...
    debug("burn_boot0_1k_mode_secure!\n");
...
__s32 burn_boot0_1k_lsb_mode_secure(__u32 read_retry_type, __u32 length,
				    __u32 Boot0_buf)
{
	__u32 i, k, j, count;
	__u8 oob_buf[32];
...
__s32 burn_boot0_lsb_FF_mode_8K(__u32 read_retry_type, __u32 length,
				__u32 Boot0_buf)
{
	...
	for (i = NAND_BOOT0_BLK_START; i < (NAND_BOOT0_BLK_START + NAND_BOOT0_BLK_CNT); i++) {
		...
		for (j = 0; j < copies_per_block; j++) {
			...
			for (k = 0; k < NAND_BOOT0_PAGE_CNT_PER_COPY; k++) {
				...
				para.block = i;
				para.page =
				    j * NAND_BOOT0_PAGE_CNT_PER_COPY + k;
				para.oobbuf = oob_buf;
...
int NAND_ReadBoot0_secure(unsigned int length, void *buf)
{
	...
	buffer = kmalloc(1024 * 512, GFP_KERNEL);
	if (buffer == NULL) {
		debug("no memory!\n");
		return -1;
	}
...
__s32 burn_uboot_in_one_blk_lsb_mode(__u32 BOOT1_buf, __u32 length)
{
	...
	buffer_temp = NAND_GetPageCache();
	if (buffer_temp == NULL) {
		debug("get buffer error!\n");
		goto error;
	}
	...
	for (i = NAND_UBOOT_BLK_START; i < (NAND_UBOOT_BLK_START + NAND_UBOOT_BLK_CNT); i++) {
		...
		for (k = 0; k < pages_per_block; k++) {
			...
			if (!Nand_Is_lsb_page(0, k))
				continue;
			if (count < pages_per_copy)
				memcpy(buffer_temp, (void *)(BOOT1_buf + count*page_size), page_size);
			para.mainbuf = buffer_temp;
			count++;
			para.oobbuf = oob_buf;
...
__s32 burn_uboot_in_many_blks_lsb_mode(__u32 BOOT1_buf, __u32 length)
{
	...
	/* burn uboot */
	page_index = 0;
	for (i = NAND_UBOOT_BLK_START;
	     i < (NAND_UBOOT_BLK_START + NAND_UBOOT_BLK_CNT); i++) {
		debug("uboot %x\n", i);
		...
		for (k = 0; k < pages_per_block; k++) {
			...
			if (page_index < pages_per_copy)
				memcpy(buffer_temp, (void *)(BOOT1_buf + page_index*page_size), page_size);
			para.mainbuf = buffer_temp;
			para.oobbuf = oob_buf;
			if (PHY_SimpleWrite(&para) < 0)
				debug("Warning. Fail in writing page %d in block %d.\n", k, i);
...
int NAND_BurnBoot1(unsigned int length, void *buf)
{
	...
	if (((lsb_page_type == 0) && (length <= block_size))
	    || ((lsb_page_type != 0)
		&& (length <= NAND_GetLsbblksize()))) {
		ret =
		    burn_uboot_in_one_blk_lsb_mode((__u32) buffer, length);
		debug("NAND_BurnBoot1, in one blk, %d\n", ret);
	} else {
		ret =
		    burn_uboot_in_many_blks_lsb_mode((__u32) buffer, length);
