                                  ...
                                  struct clock_event_device *unused)
{
	...
	/* sunxi timer maybe imprecise,
	 * we should try to fix this.
	 */
	while (retry < ARCH_TVAL_TRY_MAX_TIME) {
		pcnt = arch_counter_get_cntpct();
		arch_timer_reg_write(ARCH_TIMER_REG_TVAL, evt);
		asm volatile("mrrc p15, 2, %0, %1, c14" : "=r" (cvall), "=r" (cvalh)); 
		cval = ((u64) cvalh << 32) | cvall;
		tval = cval - pcnt;
...
static inline cycle_t arch_counter_get_cntpct(void)
{
	...
	u32 retry = 0;
		...
		cval0 = ((cycle_t) cvalh << 32) | cvall;
                isb();
                asm volatile("mrrc p15, 0, %0, %1, c14" : "=r" (cvall), "=r" (cvalh));
		cval1 = ((cycle_t) cvalh << 32) | cvall;
                delta = cval1 - cval0;
                if ((cval1 >= cval0) && (delta < ARCH_PCNT_MAX_DELTA)) {
                        /* read valid vcnt */
                        return (cycle_t)cval1;
                }
